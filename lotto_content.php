			...
			<div class="col-sm-12">
				<section class="panel">
					<div class="panel-heading color-heading">
                        Lotto Result
     <a class="btn btn-primary btn-md pull-right" href="javascript:void(0)" title="Add" onclick="add_lotto()"><i class="fa fa-plus icon-col"></i>Add Lotto Result</a>
                    </div>
					<div class="panel-body">
						<form id="form-search-content" role="form" method="post" action="lotto_content.php">
							<div class="row">
		...
		<?php
			...
			$sql_search = "select t2.title, t1.content, t1.publish_date, t1.publish_time, t1.date_created, t1.created_by
							FROM tbl_content t1
							INNER JOIN tbl_sub_category t2
							ON t1.sub_category_id_fk = t2.id
							WHERE t2.category_id_fk = '3' AND t1.publish_date BETWEEN '$start_date' AND '$end_date'";
...
$("#ucontent_table").DataTable({
  ...
  function add_lotto(){
    ...
    $('#add_title').text('Add Lotto Result'); // Set Title to Bootstrap modal title
      ...
      function save() {
                ...
                if (save_method == 'add') {
                    url = "lotto/add_lotto.php";
                } 

                var save_form = $('#form').serialize();
                // ajax adding data to database

                if ($.trim($('#sub_category_id_fk').val()) == '' || $.trim($('#content').val()) == '') {
                    ...
                    $.ajax({
                        ...
                        success: function(data) {
                            ...
                            if (data.status) //if success close modal and reload ajax table
                            {
								...
								if(save_method == "add"){
									window.location="lotto_content.php?status=1";
								}else{
									window.location="lotto_content.php?status=1&id="+data.id;
								}
	...
	<div class="modal-dialog">
		<div class="modal-content">
			<div class="modal-header">
				...
				<h4 class="modal-title" id="add_title">Add Lotto Result</h4>
			</div>
				...
				<form role="form" id="form" action="#">
					...
					<div class="form-group">
						<label>Category</label>
						 <select class="form-control" id="sub_category_id_fk" name="sub_category_id_fk" title="Lotto Category">
                            <option value="2">6-Digit</option>
                            <option value="4">3-Digit</option>
                            <option value="3">2-Digit</option>
					...
					<div class="form-group">
						<label>Content</label>
						<input type="text" class="form-control" name="content" id="content" placeholder="Separate numbers with comma [Sample: 1, 2, 3, 4, 5, 6]" required>
					</div>
					...
					<div class="form-group">
						<label>Publish Time</label>
							<select class="form-control" id="publish_time" name="publish_time" title="Lotto Publish Time">
